...
> ANSWER: They all start with https://en.wikisource.org/wiki/ as well as using the title to followup with underscores as spaces.
...
```{r}
EmDPoemsURL <- "https://en.wikipedia.org/wiki/List_of_Emily_Dickinson_poems"

EmDPoems <- (EmDPoemsURL %>%               
  read_html() %>%
  html_nodes("table")) [1] %>%   
  html_table %>%
  janitor::clean_names()
...
```{r}
TItles <- EmDPoems %>%
  mutate (titleurl = str_replace_all(First.Line..often.used.as.title, " ", "_"),
  url1 = paste0("https://en.wikisource.org/wiki/", titleurl,
  url2 = paste0(url1,"_-"))) %>%
  select(starts_with ("titleurl"),)
