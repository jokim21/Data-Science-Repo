...
> ANSWER: In the case of str_subset, it finds a pattern within a character vector. So in this case, it finds the first 6 instances where the pattern "  MACBETH" is used and prints those lines. Meanwhile, str_which finds the indices or where the pattern "  MACBETH" was found within the character vector thus printing the indices of where the pattern was found. Finally, str_detect returns a logical vector about the presence or absense of "  MACBETH" within each instance. Since we see that "  MACBETH" first appears in indice 228, it is expected to see FALSE for the first 6 instances. 
...
> ANSWER:  Regular expression syntax stipulates that a period "." is a metacharacter that matches any character. Thus, in the second example, we see that str_subset finds any instance where "MACBETH" is found. However, when we utilize the two backslashes "\\" we are able to negate this metacharacter and make it so that the period is part of the exact search. Thus, the first example finds instances where only "MACBETH." is recorded.
...
> ANSWER: "MAC[B-Z]" is finding any instances where "MAC" is followed with a capital letter that excludes the letter "A". On the other hand, "MAC[B|Z]" will only find instances where "MAC" is followed with only the capital letters B or Z (Eg MACB or MACZ). Finally, "^MAC[B-Z]" finds instances in which "MAC" is followed with a capital letter that excludes the letter "A" and is also the beginning of the text. 
