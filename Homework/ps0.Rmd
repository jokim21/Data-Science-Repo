...
Now that the necessary packages are instal?led, you should be able to Knit your document and see the results.
...
> RESPONSE: This datasarus_dozen file has 1846 rows and 3 columns due to its 3 variables, which are dataset, x, and y. 
...
> RESPONSE: The correlation coefficient between x and y is -0.06447185. It is a negative correlation which means that when one value goes up, the other value goes down. 
...
> RESPONSE: I'm not sure if there is a mathematical explanation behind it but the plot looks like a T-rex or a dinosaur. 
...
> RESPONSE: The star dataset has a negative correlation of -.0630. This is very similar to the dino dataset that we worked with before.

```{r}
star <- datasaurus %>% filter(dataset == "star") 
star %>%
  summarize(r = cor(x, y))
```
...
> RESPONSE: The plot is completely different and this data set looks like a star in coparison to a dinosaur from the dino dataset.

```{r}
ggplot(data = star, mapping = aes(y = y, x = x)) +
  geom_point()
```
...
> RESPONSE: The coeffecient is negative and is -.0683. It is not so different from the other datasets as well.

```{r}
circle <- datasaurus_dozen %>% filter(dataset == "circle") 
circle %>%
  summarize(r = cor(x, y))
```
...
> RESPONSE: The plot is now a circle or closer to a football compared to a star or a dinosaur.

```{r}
ggplot(data = circle, mapping = aes(y = y, x = x)) +
  geom_point()
```
...
> RESPONSE: the correlation coeffcient of the summaries are all generally extremely close but the graphs look extremely different in shape. Therefore it is difficult to say that the correlation coefficient is an appropriate sumary for these data sets. We would believe that they are similar graphs if we only had information about the correlation coefficient. 
